window.onload = function() {
    ...
    function renderProducts2() {
        ...
        for (let i = -1; i <= 1; i++) {
            const index = (currentIndex2 + i + meal_kit_products.length) % meal_kit_products.length;
        ...
        document.querySelectorAll('.add-to-cart').forEach(button => {
            button.addEventListener('click', function() {
                const productName2 = this.closest('.product').querySelector('h4').textContent;
    ...
    document.getElementById('prevBtn2').addEventListener('click', () => {
        currentIndex2 = (currentIndex2 - 1 + meal_kit_products.length) % meal_kit_products.length;
        renderProducts2();
    });
