window.onload = function () {
  ...
  function displayFavorites(favorites, type) {
    ...
    favorites.forEach(favorite => {
      const card = createCard(favorite, type);
      fragment.appendChild(card);
    });

    container.appendChild(fragment);  // 一次性插入到 DOM 中
    attachButtonHandlers();  // 為收藏和購物車按鈕添加事件處理
  }

  // 動態生成卡片的函數，將 HTML 結構提取出來
  function createCard(favorite, type) {
    const card = document.createElement('div');
    card.classList.add('product');
    card.dataset.productId = favorite.productId;
